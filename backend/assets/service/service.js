...
const taskService = {
  ...
  taskPutSF: function (idTask) {
    return new Promise((aceito, rejeitado) => {
      db.query(`call marcarFeito('${idTask}')`, function (error, result) {
        if (error) {
          rejeitado(error);
          return;
        }
  ...
  taskPutSN: function (idTask) {
    return new Promise((aceito, rejeitado) => {
      db.query(`call desmarcarFeito('${idTask}')`, function (error, result) {
        if (error) {
          rejeitado(error);
          return;
        }
  ...
  listPut: function (idList, nomeList) {
    return new Promise((aceito, rejeitado) => {
      db.query(
        `call modificarLista(${idList}, '${nomeList}')`,
        (error, result) => {
          if (error) {
            rejeitado(error);
            return;
