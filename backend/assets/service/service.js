const db = require('../database/conn');
    ...
    taskPost: function(id, nomeTarefa, descricao, categoria){
        return new Promise((aceito, rejeitado) => {
            db.query(`insert into TAREFAS (nomeTarefa, descricao, fk_IdLista, tipo) value ('${nomeTarefa}','${descricao}', '${id}', '${tipo}')`, (erro, results) => {
    ...
    listPost: function(id, nomeLista){
        return new Promise((aceito, rejeitado) => {
            db.query(`insert into LISTAS (nomeLista) value ('${nomeLista}')`, (erro, results) => {
                if (erro) {rejeitado(erro); return;}
                aceito('Inserido com Sucesso');
            });
        });
